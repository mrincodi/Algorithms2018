...
// First approach. Could have been done on half the code.
// 30m
public class ElectronicsShop {
	...
	static int getMoneySpent(int[] keyboards, int[] drives, int b) {
		...
		for ( int keyboardsPointer = 0; keyboardsPointer < keyboards.length; keyboardsPointer++){
			...
			if ( thisSum < b ){
				drivesPointer++;
				while (drivesPointer < drives.length ){

					int newSum = keyboards [ keyboardsPointer ] + drives [ drivesPointer ];
					if ( newSum == b ) return b;
