...
// Again, failing for some strange reason definitely related to having 2 
// contiguous kids with the same ranking. 
// OK, so I discovered it's InterviewBit's fault.
// This code gives 930 here and 932 in  InterviewBit.
public class DistributeCandySecondAttempt {

	public int candy(ArrayList<Integer> A) {

		TreeMap <Integer, ArrayList <Integer> >t  = new TreeMap <Integer, ArrayList <Integer> > ();

		int [] resultArray = new int [A.size() + 2 ];
		for ( int i = 0; i < A.size(); i++){
			int ranking = A.get(i);

			ArrayList <Integer> rankingPositions = t.containsKey(ranking)?t.get(ranking):new ArrayList <Integer> ();
			rankingPositions.add(i);
			t.put(ranking,rankingPositions);
		...
		while (iter.hasNext()){
			int ranking = iter.next();
			
			ArrayList <Integer> rankingPositions = t.get(ranking);
			
			
			for ( int i = 0; i < rankingPositions.size(); i++){
				int position = rankingPositions.get(i)+1;
				
				resultArray[position]=Math.max(resultArray[position - 1],resultArray[position + 1]) + 1;
			}
		}
		
		for (int ranking:resultArray)
			result += ranking;
