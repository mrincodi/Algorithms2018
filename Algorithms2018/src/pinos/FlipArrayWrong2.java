		...
		for ( int i = 1; i < list.size();i++){
			for (int w = 0; w <= W; w++){
				if ( w < list.get(i) )
					...
					m[i][w] = Math.min(m[i-1][w], m[i-1][w-list.get(i)] + 1);
				}
			}
		}
		
		return m[list.size()][W];
...
//		return numChanges;
	...
	public static void main(String[] args) {
		...
		ArrayList < Integer> values = new ArrayList < Integer> (Arrays.asList( 9,6 ));		
		int result = new FlipArrayWrong2().solve(values);
